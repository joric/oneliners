...
The solution above is not very reliable, because tests and environment may change, but it's among the fastest.
...
(functions return `None`). To set a key for the list or for the dictionary, you can use `setattr` or `setitem` function, e.g. `c[x]=1` is the same as `setitem(c,x,1)`.
