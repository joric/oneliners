...
class Solution:
    def findDiagonalOrder(self, n: List[List[int]]) -> List[int]:
        d=[];[(i+j>=len(d)and d.append([]),d[i+j].append(a))for i,r in enumerate(n)for j,a in enumerate(r)];return[a for r in d for a in reversed(r)]

class Solution:
    def findDiagonalOrder(self, n: List[List[int]]) -> List[int]:
        d=defaultdict(list);[d[i+j].append(a)for i,r in enumerate(n)for j,a in enumerate(r)];return[a for r in d for a in d[r][::-1]]
