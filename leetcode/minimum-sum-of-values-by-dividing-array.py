...
class Solution:
    def minimumValueSum(self, a: List[int], v: List[int]) -> int:
        n,m=len(a),len(v);return(-1,r:=(f:=cache(lambda b,j,k:0 if k==m and j==n else inf if k==m or j==n or (b:=b&a[j])<v[k]else min(f(b,j+1,k),a[j]+f((1<<32)-1,j+1,k+1))if b==v[k]else f(b,j+1,k)))((1<<32)-1,0,0))[r<inf]
