...
class Solution:
    def maximumScore(self, g: List[List[int]]) -> int:
        a=[[0,*accumulate(v)]for v in zip(*g)];return(f:=cache(lambda i,h,b:i<len(g)and max(f(i+1,j,j>=h)+(i and[b*(a[i-1][j]-a[i-1][h]),a[i][h]-a[i][j]][j<h])for j in range(len(g)+1))))(0,0,1)
