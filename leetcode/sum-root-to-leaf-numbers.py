...
class Solution:
    def sumNumbers(self, root: TreeNode) -> int:
        return (f:=lambda x,v:x and (v:=x.val+10*v,f(x.left,v)+f(x.right,v) if (x.left or x.right) else v)[1] or 0)(root,0)

class Solution:
    def sumNumbers(self, root: TreeNode) -> int:
        return (f:=lambda x,v:x and (lambda l,r,v:(l or r) and f(l,v)+f(r,v) or v)(x.left,x.right,x.val+10*v) or 0)(root,0)
