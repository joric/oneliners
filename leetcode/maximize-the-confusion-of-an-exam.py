...
class Solution:
    def maxConsecutiveAnswers(self, s: str, k: int) -> int:
        m=r=0;c=Counter();[(c.update({s[i]:1}),m:=max(m,c[s[i]]),r-m<k and(r:=r+1)or c.update({s[i-r]:-1}))for i in range(len(s))];return r
...
class Solution:
    def maxConsecutiveAnswers(self, s: str, k: int) -> int:
        m,c,n=0,Counter(),len(s);[(c.update({s[i]:1}),max(c.values())<=i-m-k and(c.update({s[m]:-1}),m:=m+1))for i in range(n)];return n-m
...
class Solution:
    def maxConsecutiveAnswers(self, s: str, k: int) -> int:
        r,c=0,Counter();[(c.update({s[i]:1}),r-k<max(c.values())and(r:=r+1)or c.update({s[i-r]:-1}))for i in range(len(s))];return r
