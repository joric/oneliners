...
class Solution:
    def averageOfSubtree(self, r: Optional[TreeNode]) -> int:
        return(f:=lambda x,v:x and(l:=f(x.left,v),r:= f(x.right,v),s:=l[0]+r[0]+x.val,t:=l[1]+r[1]+1,(s,t,l[2]+r[2]+(s//t==x.val)))[-1]or(0,0,0))(r,0)[2]
...
class Solution:
    def averageOfSubtree(self, r: Optional[TreeNode]) -> int:
        return(t:=lambda f,n:n and f(n)+t(f,n.left)+t(f,n.right)or 0)(lambda n:n.val==t(lambda m:m.val,n)//t(lambda m:1,n),r)

class Solution:
    def averageOfSubtree(self, r: Optional[TreeNode]) -> int:
        a=[];(f:=lambda n:n and[s:=f(n.left)+f(n.right)+n.val+1j,a.append(n.val==s.real//s.imag)][0]or 0j)(r);return sum(a)

class Solution:
    def averageOfSubtree(self, r: Optional[TreeNode]) -> int:
        b=1001;return(f:=lambda n:n and(s:=f(n.left)+f(n.right)+n.val*b*b+b)+(n.val==s//(s//b%b*b*b))or 0)(r)%b
