...
Just FYI, this repository is not only README.md, it's also about a thousand solutions [here](./leetcode).
...
Leetcode also has `serialize` and `deserialize` for lists and trees:
...
You can use `numpy.convolve` for sliding windows, it's usually shorter and than reduce or list comprehension:
...
class Solution:
    def maxSatisfied(self, c: List[int], g: List[int], m: int) -> int:
        t,a=0,[*map(mul,c,g)];[t:=(t,w:=sum(a[i:i+m]))[w>t]for i in range(len(c)-m+1)];return t+sum(c)-sum(a)
