...
class Solution:
    def assignEdgeWeights(self, e: List[List[int]], q: List[List[int]]) -> List[int]:
        s,m=setitem,defaultdict;g,h,d,l,f=m(set),m(set),{},{},[*range(len(e)+2)];[a[u].add(v)or a[v].add(u)for a,b in((g,e),(h,q))for u,v in b];r=lambda x:x if x==f[x]else s(f,x,r(f[x]))or f[x];t=lambda u,w:(s(d,u,w),[(t(v,w+1),s(f,v,u))for v in g[u]if v not in d],[s(l,(u,v),z:=r(v))or s(l,(v,u),z)for v in h[u]if v in d]);t(1,0);return[i and pow(2,i-1,10**9+7)for i in(d[x]+d[y]-2*d[l[x,y]]for x,y in q)]
