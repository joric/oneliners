...
You can also use `__import__('modulename').varname` to import from missing modules. Example:
...
Also it's possible to unpack values in a list comprehension i.e. `[f(a,b) a,b for [ab]]` (walrus operator can't unpack tuples).
...
It seems to be the most useful operator here, most oneliners would be impossible to do without it (or rather, very hard, with nested lambdas/y-combinator).
