...
class LFUCache:
    ...
    def put(s,k,v):
        (f:=s.k[k],s.get(k),setitem(s.f[f+1],k,v)) if k in s.k else s.c and (len(s.k)==s.c and (l:=min(s.f),p:=s.f[l].popitem(last=0)[0],s.k.pop(p), not s.f[l] and s.f.pop(l)),setitem(s.f[1],k,v),setitem(s.k,k,1))
