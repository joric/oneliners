...
# https://leetcode.com/problems/magic-squares-in-grid/solutions/7449444/one-line-solution-by-xxxxkav-v058/

class Solution:
    def numMagicSquaresInside(self, g: List[List[int]]) -> int:
        return sum(''.join(map(str,g[i][j:j+3]+g[i+1][j:j+3]+g[i+2][j:j+3]))in{'276951438','294753618','438951276','492357816','618753294','672159834','816357492','834159672'}for i,j in product(range(len(g)-2), range(len(g[0])-2)))
...
class Solution:
    def numMagicSquaresInside(self, g: List[List[int]]) -> int:
        return sum(g[i+1][j+1]==5>g[i][j]%2+4!=''.join(str(g[i+x//3][j+x%3])for x in(0,1,2,5,8,7,6,3))in'43816729'*2+2*'92761834'for i in range(len(g)-2)for j in range(len(g[0])-2))
...
class Solution:
    def numMagicSquaresInside(self, g: List[List[int]]) -> int:
        return sum(r[j+1]==5>q[j]%2+4!='%d'*8%(*q[j:j+3],r[j+2],*s[j:j+3][::-1],r[j])in(t:='43816729'*2)+t[::-1]for q,r,s in zip(g,g[1:],g[2:])for j in range(len(q)-2))
