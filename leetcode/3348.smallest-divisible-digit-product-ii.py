...
class Solution:
    def smallestNumber(self,n:str,t:int)->str:
        c=len(n);f=lambda x,y,z=9:'1'*y if z<2 else f(x,y,z-1)if x%z else f(x//z,y-1,z)+str(z);h=[t];[h.append(h[-1]//gcd(h[-1],int(x)))for x in n.split('0')[0]];return'-1'if prod(map(int,f(t,0)))<t else n if len(h)>c and h[-1]==1 else next((n[:i]+str(j)+k for i in range(n.find('0')%c,-1,-1)for j in range(int(n[i])+1,10)if len(k:=f(h[i]//gcd(h[i],j),c-i-1))==c-i-1),f(t,c+1))

class Solution:
    def smallestNumber(self,n:str,t:int)->str:
        c=len(n);f=lambda x,y,z=9:'1'*y if z<2else f(x,y,z-1)if x%z else f(x//z,y-1,z)+str(z);h=[t];[h.append(h[-1]//gcd(h[-1],int(x)))for x in n.split('0')[0]];return'-1'if t>prod(map(int,f(t,0)))else n if c<len(h)and 1==h[-1]else next((n[:i]+str(j)+k for i in range(n.find('0')%c,-1,-1)for j in range(int(n[i])+1,10)if len(k:=f(h[i]//gcd(h[i],j),c-i-1))==c-i-1),f(t,c+1))
