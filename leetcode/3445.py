...
class Solution:
    def maxDifference(self, s: str, k: int) -> int:
        q,f,t=-inf,lambda a,b:(a&1)<<1|b&1,'01234';[(l:=-1,d:=[inf]*4,u:=0,v:=0,x:=0,y:=0,[(u:=u+(c==a),v:=v+(c==b),all(r-l>=k and v-y>=2 and(setitem(d,m:=f(x,y),min(d[m],x-y)),x:=x+(s[l:=l+1]==a),y:=y+(s[l]==b))for _ in s),inf>d[m:=f(u,v)^2]and(q:=max(q,u-v-d[m])))for r,c in enumerate(s)])for a in t for b in t if a!=b];return q
