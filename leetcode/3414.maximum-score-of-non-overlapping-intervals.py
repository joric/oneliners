...
class Solution:
    def maximumWeight(self, a: List[List[int]]) -> List[int]:
        d=sorted([e,s,w,i]for i,(s,e,w)in enumerate(a));f=cache(lambda i,k:i*k and min(f(i-1,k),((t:=f(bisect_left(d,[(x:=d[i-1])[1]]),k-1))[0]-x[2],sorted([x[3]]+t[1])))or(0,[]));return f(len(a),4)[1]
