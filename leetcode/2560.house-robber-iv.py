...
class Solution:
    def minCapability(self, a: List[int], k: int) -> int:
        return bisect_right(range(max(a)),0,key=lambda c,s=0:sum(s:=s==0and v<=c for v in a)>=k)

class Solution:
    def minCapability(self, a: List[int], k: int) -> int:
        return bisect_right(range(10**9),0,key=lambda c,s=0:sum(s:=s==0and v<=c for v in a)>=k)
