...
Uploading all tests is not a very reliable solution, because tests may change in the future, but it's usually the fastest:
...
To set a key for the list or for the dictionary, you can use `operator.setitem` function, e.g. `c[x]=1` is the same as `setitem(c,x,1)`.
