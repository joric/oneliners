...
class Solution:
    def minimumCost(self, a: List[int], k: int, d: int) -> int:
        n,s,m=len(a),SortedList(a[1:1+d]),inf;c=sum(s[:k-2]);[(m:=min(m,c:=c+(k-2<s.bisect(a[i])and s[k-2]or a[i])),s.add(a[i]),c:=c-(k-2<s.bisect(a[i-d])and s[k-2]or a[i-d]),s.remove(a[i-d]))for i in range(1+d,n)];return a[0]+m
...
class Solution:
    def minimumCost(self, a: List[int], k: int, d: int) -> int:
        s,t=SortedList(a[1:1+d]),k-2;b,c=s.bisect,sum(s[:t]);return a[0]+min((c:=c+(t<b(u)and s[t]or u),s.add(u),c:=c-(t<b(v)and s[t]or v),s.remove(v))[0]for u,v in zip(a[1+d:],a[1:]))

class Solution:
    def minimumCost(self, a: List[int], k: int, d: int) -> int:
        s,m,t=SortedList(a[1:d+1]+[inf]),inf,k-2;c=sum(s[:t]);[(m:=min(m,c:=c+min(u,s[t])),s.add(u),c:=c-min(v,s[t]),s.remove(v))for u,v in zip(a[d+1:],a[1:])];return a[0]+m
