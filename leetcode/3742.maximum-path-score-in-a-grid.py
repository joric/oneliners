...
class Solution:
    def maxPathScore(self, g: List[List[int]], k: int) -> int:
        n,m=len(g),len(g[0]);f=cache(lambda i,j,c:v+(i+j+2<n+m and max(f(i+1,j,t),f(i,j+1,t)))if i-n<0>j-m and(t:=c+(0<(v:=g[i][j])))<=k else-inf);r=max(-1,f(0,0,0));f.cache_clear();return r
