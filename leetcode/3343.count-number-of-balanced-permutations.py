...
class Solution:
    def countBalancedPermutations(self, s: str) -> int:
        c,t,n=Counter(m:=[*map(int,s)]),sum(m),len(s)//2;return~t%2 and(f:=cache(lambda i,o,e,b:not(o|e|b)or min(i,o,e,b)>=0 and sum(comb(o,j)*comb(e,c[i]-j)*f(i-1,o-j,e-c[i]+j,b-i*j)for j in range(c[i]+1))%(10**9+7)))(9,len(s)-n,n,t//2)
