...
class Solution:
    def maximumDetonation(self, b: List[List[int]]) -> int:
        g=lambda x,y:abs(x[0]-y[0])**2+abs(x[1]-y[1])**2<=x[2]**2
        f=lambda i,v:v.add(i) or 1+sum(f(j,v) for j in g[i] if j not in v)
        g={i:[j for j in range(len(b)) if g(b[i],b[j])] for i in range(len(b))}
        return max(f(i,set()) for i in range(len(b)))
