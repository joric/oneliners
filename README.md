...
To set a key, you can use a global `setitem` function (`operator.setitem`), e.g. `c[x]=1` is the same as `setitem(c,x,1)`.
...
Also see swap function here (but it's pretty long):
...
Sometimes you can even skip `itemgetter(0)` by converting an argument to a tuple (15 characters shorter).
...
* You can replace `-1` or `not` with bitwise negation `~-` to save on space (1-5 characters shorter).
