...
class Solution:
    def maxTotalValue(self, a: list[int], k: int) -> int:
        n=len(a);b=int.bit_length;d=~-b(n);h=[];p=heappush;r=range;z=[([[x]+[0]*d for x in a],m)for m in(min,max)];f=lambda l,y:sub(*[m(t[l][j:=~-b(y-l+1)],t[y-2**j+1][j])for t,m in z]);[setitem(t[i],j+1,m(t[i][j],t[i+2**j][j]))for j in r(d)for i in r(n-2**-~j+1)for t,m in z];[p(h,(f(l,n-1),l,n-1))for l in r(n)];return-sum((lambda v,l,y:y>l and p(h,(f(l,y-1),l,y-1))or v)(*heappop(h))for w in r(k))
