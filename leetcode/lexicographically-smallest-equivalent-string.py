...
class Solution:
    def smallestEquivalentString(self, s1: str, s2: str, baseStr: str) -> str:
        return (p:={c:c for c in set(s1+s2)}) and any(p.__setitem__(max((f:=lambda c:c if c not in p or p[c]==c else f(p[c]))(a),f(b)),min(f(a),f(b))) for a,b in zip(s1,s2)) or ''.join(f(c) for c in baseStr)
