...
class Solution:
    def stoneGameIII(self, v: List[int]) -> str:
        n=len(v);o=[0,0,0];any(setitem(o,i%3,max(sum(v[i:i+k])-o[(i+k)%3]for k in range(1,4)))for i in range(n-1,-1,-1));return(('Bob','Alice')[o[0]>0],'Tie')[o[0]==0]

class Solution:
    def stoneGameIII(self, v: List[int]) -> str:
        r=(f:=cache(lambda i:i<len(v)and max(sum(v[i:i+k])-f(i+k)for k in range(1,4))))(0);return('Bob','Tie','Alice')[(r>0)-(r<0)+1]
